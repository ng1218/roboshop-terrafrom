...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-pull -i localhost, -U https://github.com/ng1218/roboshop-ansible roboshop.yml -e role_name=${local.role_name} -e app_name=${var.name} -e env=dev -e token=${var.token}"
    ]
  }
}
resource "azurerm_dns_a_record" "dns_record_resource_group" {
  ...
  resource_group_name   = var.dns_record_resource_group
  ttl                   = 3
